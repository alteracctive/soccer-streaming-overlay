...
export function render(container: HTMLElement) {
  ...
  let currentPeriodLimit = 45; // Default fallback
    ...
    <div class="modal-overlay" id="set-time-modal" style="display: none;">
      <div class="modal-content" style="max-width: 360px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 16px; margin: 20px 0;">
            ...
            <p style="font-size: 11px; text-align: center; margin: 0; opacity: 0.7;">
              Sets the total time. If Main exceeds the period limit, the excess flows into Additional.
            </p>
        ...
        <div class="card-content">
          ...
          <div class="player-side">
            <div class="player-grid-container" id="player-grid-container-a">
              <div class="player-grid" id="player-grid-a" data-team="teamA"></div>
            </div>
        ...
        <div class="card-content">
          ...
          <div class="player-side">
            <div class="player-grid-container" id="player-grid-container-b">
              <div class="player-grid" id="player-grid-b" data-team="teamB"></div>
            </div>
  ...
  const addTimerMinInput = container.querySelector('#add-timer-min') as HTMLInputElement; // New
  const addTimerSecInput = container.querySelector('#add-timer-sec') as HTMLInputElement; // New
  ...
  const initPeriods = async () => {
    try {
        ...
        if (config && config.currentPeriod) {
             ...
             periodSelect.value = allPeriods[0].name;
        }
        updateEndTimeLabel();
    } catch (e) {
  ...
  const updateEndTimeLabel = () => {
      ...
      if (period) {
          currentPeriodLimit = period.endTime; // Update global tracker
  ...
  const updateUI = () => {
    ...
    if (totalSeconds <= limitSeconds) {
        ...
        // Overflow Case
        timerMainEl.textContent = formatTime(limitSeconds); // Clamp main to limit
        timerAddEl.textContent = `+ ${formatTime(totalSeconds - limitSeconds)}`; // Show excess
    }
  ...
  saveSetTimeBtn.addEventListener('click', () => {
    ...
    const addTotal = (addMins * 60) + addSecs;

    // Send sum to backend
    timerControls.set(mainTotal + addTotal);
