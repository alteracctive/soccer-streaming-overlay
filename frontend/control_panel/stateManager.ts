...
let appState: {
  ...
  isPlayersListVisible: boolean; // Kept for backward compat in other files if needed, but logic updated
  ...
  isPlayersListVisible: false, // Legacy fallback
...
// --- Updated Helper ---
function updatePlayersListVisibility(data: { isVisibleA: boolean, isVisibleB: boolean }) {
  appState.isPlayersListVisibleA = data.isVisibleA;
  appState.isPlayersListVisibleB = data.isVisibleB;
  appState.isPlayersListVisible = data.isVisibleA || data.isVisibleB; // Aggregate for simple checks
...
function connectWebSocket() {
  ...
  ws.onmessage = (event) => {
    ...
    else if (message.type === 'players_list_visibility') updatePlayersListVisibility(message); // <-- Handle new format
...
// --- Updated Player List API Functions ---
...
export async function uploadJson(fileName: string, jsonData: string) { await post('/api/json/upload', { file_name: fileName, json_data: jsonData }); }